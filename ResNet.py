...
MODEL_PATH = os.path.join(BASE_OUTPUT, "ijcai_verify/models/aiims_resnet50.pt")
...
train_dataloader_b = torch.utils.data.DataLoader(dataset_b, batch_size=32, shuffle=True, num_workers=30)
valid_dataloader_b = torch.utils.data.DataLoader(dataset_v, batch_size=32, shuffle=True, num_workers=12)
test_dataloader_b = torch.utils.data.DataLoader(dataset_v, batch_size=1, shuffle=False, num_workers=0)
...
model.fc = nn.Sequential(nn.Linear(nr_filters, 1024), nn.BatchNorm1d(1024), nn.ReLU(inplace=True),nn.Linear(1024,512), nn.Dropout(0.5), nn.Linear(512, 128), nn.Linear(128, 1))
# #freeze all params
...
torch.save(best_model_wts, 'ijcai_verify/models/aiims_resnet50.pt')
...
plt.savefig('ijcai_verify/graphs/aiims_resnet501.jpg')
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('ijcai_verify/graphs/aiims_resnet502.jpg')
